...
TestCase {
    name: "MasterPasswordFlow"
    property int waitMs: 3500
    ...
    function test_create_and_unlock_with_same_password() {
        var dbUrl = "sqlite:///tmp/zann-kde-ui-test-" + Date.now() + ".sqlite";
        var root = createApp();
        ...
        appModel.debug_create_kv_item("kv/test", "foo", "bar");
