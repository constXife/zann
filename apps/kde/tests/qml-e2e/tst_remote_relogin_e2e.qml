...
TestCase {
    name: "RemoteReloginE2E"
    property int waitMs: 20000
    ...
    function test_remote_relogin_with_master_password() {
        var root = createApp();
        var appModel = findByName(root, "appModel");
        if (appModel.debug_get_env("ZANN_E2E_MODE") !== "real") {
            root.visible = false;
            root.destroy();
            skip("remote e2e disabled");
        }
        ...
        var dbUrl = appModel.debug_make_temp_db_url("remote-e2e");
