...
const props = defineProps<{
  ...
  isExpanded: (field: FieldRow) => boolean;
  isLongValue: (field: FieldRow) => boolean;
  toggleExpanded: (field: FieldRow) => void;
  openLink: (field: FieldRow) => void;
  openCharView: (field: FieldRow) => void;
  handleCopy: (field: FieldRow) => void;
  copiedField: string | null;
  diffStatus: (field: FieldRow) => "modified" | "added" | "removed" | "same";
  diffPreviousValue: (field: FieldRow) => string;
  diffPreviousMasked: (field: FieldRow) => boolean;
      ...
      <div class="grid grid-cols-[180px,1fr] gap-4 items-start">
        ...
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0 flex-1">
            <button
              ...
              :class="[
                ...
                timeTravelActive && diffStatus(field) === 'modified' ? 'bg-amber-500/10 rounded-md' : '',
              ]"
          ...
          <div class="flex items-center gap-1 opacity-100">
            ...
            >
              <svg v-if="!(altRevealAll || isRevealed(field.path))" class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
