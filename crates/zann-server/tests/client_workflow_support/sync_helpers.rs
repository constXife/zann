...
pub(super) async fn apply_push_applied(
    ...
    for entry in entries {
        let Some(item_id) = entry["item_id"]
            .as_str()
        ...
        let Some(mut item) = repo
            ...
            .await
            .expect("get applied item")
        else {
            continue;
        };
        item.version = entry["seq"].as_i64().expect("applied sequence");
        item.updated_at =
            DateTime::parse_from_rfc3339(entry["updated_at"].as_str().expect("applied updated_at"))
                .expect("valid applied updated_at")
                .with_timezone(&Utc);
        item.deleted_at = entry["deleted_at"].as_str().map(|value| {
            DateTime::parse_from_rfc3339(value)
                .expect("valid applied deleted_at")
                .with_timezone(&Utc)
        });
        item.sync_status = if item.deleted_at.is_some() {
            SyncStatus::Tombstone
        } else {
            SyncStatus::Synced
        };
        repo.update(&item).await.expect("update applied item");
    }
}
