...
async fn health_includes_component_statuses() {
    ...
    assert!(components.get("db").is_some());
    assert!(components.get("db_pool").is_some());
    assert!(components.get("kdf").is_some());
    assert!(components.get("oidc").is_some());

    let oidc_status = components
        .get("oidc")
