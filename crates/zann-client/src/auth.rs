...
use crate::state::{ClientState, IdentityConfig, PendingLogin, PendingLoginResult, TokenEntry};
use crate::types::{
...
pub fn empty_oidc_config() -> OidcConfigResponse {
    OidcConfigResponse {
        issuer: String::new(),
...
pub fn empty_oidc_discovery() -> OidcDiscovery {
    OidcDiscovery {
...
pub fn oidc_status_error(login_id: &str, message: impl Into<String>) -> OidcLoginStatusResponse {
...
pub fn oidc_status_fingerprint_changed(
    login_id: &str,
    old_fingerprint: &str,
    new_fingerprint: &str,
) -> OidcLoginStatusResponse {
...
pub fn fingerprint_change_for_context(
    state: &ClientState,
    context_name: &str,
    new_fingerprint: &str,
) -> Option<String> {
...
pub fn context_name_for_server_id(state: &ClientState, server_id: &str) -> Option<String> {
    let config = load_config(&state.root).ok()?;
    config
        .contexts
        .iter()
...
pub fn update_pending_login_for_fingerprint(
    state: &ClientState,
    server_url: &str,
    login_id: &str,
    new_fingerprint: &str,
...
pub async fn fetch_personal_status(
    server_url: &str,
    access_token: &str,
...
pub async fn apply_login_context(
    state: &ClientState,
    server_url: &str,
    result: &PendingLoginResult,
) -> Result<Option<String>, String> {
...
pub fn migrate_context_for_server_id(
